...
int main()
{
        ...
        double *paramters_noised[5] = {pose0_noised.parameterPtr(),
                                       pose1_noised.parameterPtr(),
                                       pose2_noised.parameterPtr(),
                                       pose3_noised.parameterPtr(),
                                       &param_dt};
        ...
        std::cout << "AnaliJacobian_minimal3: " << std::endl
                  << AnaliJacobian3_min << std::endl;

        Eigen::Matrix<double, 6, 1> numJacobian_min4;
        numbDifferentiator.df_r_xi<6, 1>(paramters_noised, 4, numJacobian_min4.data());

        std::cout << "numJacobian_min4: " << std::endl
                  << numJacobian_min4 << std::endl;
        std::cout << "AnaliJacobian_minimal4: " << std::endl
                  << AnaliJacobian4_min << std::endl;
        /*
         * Optimization test
         */
        ...
        problem.AddResidualBlock(quatSplineError0, loss_function,
                                 pose0.parameterPtr(), pose1.parameterPtr(),
                                 pose2.parameterPtr(), pose3.parameterPtr(),
                                 &est_dt);
        problem.AddResidualBlock(quatSplineError1, loss_function,
                                 pose0.parameterPtr(), pose1.parameterPtr(),
                                 pose2.parameterPtr(), pose3.parameterPtr(),
                                 &est_dt);
        problem.AddResidualBlock(quatSplineError2, loss_function,
                                 pose0.parameterPtr(), pose1.parameterPtr(),
                                 pose2.parameterPtr(), pose3.parameterPtr(),
                                 &est_dt);
        problem.AddResidualBlock(quatSplineError3, loss_function,
                                 pose0.parameterPtr(), pose1.parameterPtr(),
                                 pose2.parameterPtr(), pose3.parameterPtr(),
                                 &est_dt);

        problem.AddResidualBlock(quatSplineError4, loss_function,
                                 pose0.parameterPtr(), pose1.parameterPtr(),
                                 pose2.parameterPtr(), pose3.parameterPtr(),
                                 &est_dt);
        problem.AddResidualBlock(quatSplineError5, loss_function,
                                 pose0.parameterPtr(), pose1.parameterPtr(),
                                 pose2.parameterPtr(), pose3.parameterPtr(),
                                 &est_dt);
