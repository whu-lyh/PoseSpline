...
TEST(Pose, operations){
    ...
    for (size_t i = 0; i < 100; ++i) {
        ...
        T_BC.setRandom();
        // Test inverse
        GTEST_ASSERT_LT(((T_AB * T_AB.inverse()).Transformation() - Eigen::Matrix4d::Identity()).norm(),1e-8);
        // Test composition
        ...
        for (size_t i = 0; i < 6; ++i) {
            ...
            // why 2.0 : plus-minus, so 2 times of update
            jacobian_numDiff.block<3, 1>(0, i) = (T_AB_p.r() - T_AB_m.r()) / (2.0 * dp);
            jacobian_numDiff.block<4, 1>(3, i) = (T_AB_p.q() - T_AB_m.q()) / (2.0 * dp);
        }
        // poseLocalParameter jacobian has same idea with T_AB(pose)
        ...
        GTEST_ASSERT_LT((local_lift_jacobian*local_plus_jacobian - Eigen::Matrix<double,6,6>::Identity()).squaredNorm() , 1e-8);
        ...
        GTEST_ASSERT_LT((lift_jacobian * jacobian - Eigen::Matrix<double, 6, 6>::Identity()).norm() , 1e-8);
    }
}
...
TEST (Pose, QuaternionPlusList) {
    ...
    for (int i =0; i < 1000; i++) {
        // construct a random quaternion and normalize it
        ...
        //std::cout<<"Liftjac* pluJac: "<<std::endl<<liftJacobian*plusJacobian<<std::endl;
        //std::cout << "liftJacobian: \n" << liftJacobian << std::endl;
        //std::cout << "plusJacobian: \n" << plusJacobian << std::endl;
        // why lift times plus equal to 1?
...
TEST (Pose, PosePlusLift) {
    ...
    for (int i =0; i < 1000; i++) {
        ...
        //std::cout << "liftJacobian: \n" << liftJacobian << std::endl;
        //std::cout << "plusJacobian: \n" << plusJacobian << std::endl;
        GTEST_ASSERT_LT((liftJacobian*plusJacobian - Eigen::Matrix<double,6,6>::Identity()).norm(), 1e-8);
        // pose jacobian formulation is inherited from quaternion class jacobian
        ...
        GTEST_ASSERT_LT((liftJacobian.bottomRightCorner(3,4) - quatLiftJacobian).norm(), 1e-8);
        GTEST_ASSERT_LT((plusJacobian.bottomRightCorner(4,3) - quatPlusJacobian).norm(), 1e-8);
    }
}
